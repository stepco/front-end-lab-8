...
function fighter(propertiesObj) {
  let theFighter = propertiesObj;

  let theFighterHistory = {
    wins: 0,
    loses: 0,
  };
  ...
  function fight(enemy) {
    ...
    if (theFighter.attack > enemy.getStats().hp) {
      return true;
    }
    ...
    enemy.getStats().hp -= theFighter.attack;

    if (enemy.getStats().hp === 0) {
      theFighterHistory.wins++;
